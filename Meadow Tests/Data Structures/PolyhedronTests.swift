...
class PolyhedronTests: XCTestCase {
    ...
    func testPolyhedronEquality() {
        ...
        let p0 = Polytope(x: 0.0, y: World.Y(y: 1), z: 0.0)
        let p1 = Polytope(x: 0.0, y: World.Y(y: 0), z: 0.0)
        
        let p2 = Polytope(x: 0.0, y: World.Y(y: 3), z: 0.0)
        let p3 = Polytope(x: 0.0, y: World.Y(y: 2), z: 0.0)
        
        let p4 = Polytope(x: 0.0, y: World.Y(y: -2), z: 0.0)
        let p5 = Polytope(x: 0.0, y: World.Y(y: -3), z: 0.0)
        
        let reference = Polyhedron(upperPolytope: p0, lowerPolytope: p1)
        
        let above = Polyhedron(upperPolytope: p2, lowerPolytope: p3)

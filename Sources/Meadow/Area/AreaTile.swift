...
public class AreaTile: NSObject, Codable, Collapsable, Renderable, Responder, SceneGraphNode, Soilable, Traversable, Updatable {
    ...
    public var category: Int { SceneGraphCategory.areaTile.rawValue }
    
    public var neighbours: [Cardinal : AreaTile] = [:] {
    ...
    var tilesetTile: AreaTilesetTile? = nil
...
extension AreaTile {
    ...
    public static func == (lhs: AreaTile, rhs: AreaTile) -> Bool {
...
extension AreaTile {
    ...
    func add(neighbour: AreaTile, cardinal: Cardinal) {
...
extension AreaTile {
    ...
    func render(position: Vector) -> [Polygon] {
        ...
        guard let edges = meadow?.world.tilemaps.area.edgeset.edges(with: tileType) else { return polygons }
...
extension AreaTile {
    ...
    func collapse() {
        ...
        guard let tilemap = meadow?.world.tilemaps.area, tilesetTile == nil else { return }
        ...
        for cardinal in Cardinal.allCases.shuffled(using: &rng) {
            ...
            var t0: AreaTileType? = d0?.tileType == tileType ? tileType : nil
            var t1: AreaTileType? = d1?.tileType == tileType ? tileType : nil
            
            if d0 != nil {
                
                t0 = !neighbour.traversable(cardinal: c0) ? tileType : t0
            }
            
            if d1 != nil {
                
                t1 = !neighbour.traversable(cardinal: c1) ? tileType : t1
            }
